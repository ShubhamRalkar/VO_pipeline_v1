...
fprintf('========================\n\n');
%% ============================================================
%  MODULE 1 — CAMERA FRONT-END PARAMETERS
%  ============================================================
...
% Feature rendering (how landmarks appear in synthetic frame)
blob_radius  = 4;       % pixels — size of rendered landmark dot
blob_sigma   = 2.0;     % Gaussian blur sigma for realistic appearance
min_depth    = 0.5;     % m — minimum visible depth
max_depth    = 50.0;    % m — maximum visible depth

% ORB parameters
orb_num_points   = 500;   % max keypoints per frame
orb_scale_factor = 1.2;   % scale factor between pyramid levels
orb_num_levels   = 8;     % pyramid levels

% KLT parameters
klt_block_size      = [31 31];   % tracking window size
klt_max_iterations  = 30;        % KLT solver iterations
klt_num_levels      = 3;         % pyramid levels
klt_max_error       = 5.0;       % pixels — max bidirectional error

% RANSAC parameters
ransac_max_distance    = 1.5;    % pixels — epipolar tolerance
ransac_confidence      = 99.0;   % % confidence
ransac_max_iterations  = 500;

% Keyframe thresholds
kf_min_features     = 80;    % trigger new keyframe below this
kf_max_translation  = 0.3;   % m — trigger if moved this far
kf_max_rotation     = 5.0;  % deg — trigger if rotated this far
% Define EKF initial state estimate as a 15-state Simulink Parameter
x0_ekf = Simulink.Parameter;
x0_ekf.Value = zeros(15, 1);
x0_ekf.DataType = 'double';
x0_ekf.Dimensions = [15 1];
%% ============================================================
